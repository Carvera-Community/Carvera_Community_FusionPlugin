...
This specification is derived from the current repository source, README behavior descriptions, and owner clarifications made during review. The initial host suite contains 13 tests covering line recognition and writing, rapid parsing and effective-distance analysis, and session-only setting persistence. `python3 -m compileall -q commands Tests` and `git diff --check` provide additional syntax and patch-format evidence. None of this proves Fusion API compatibility, correct post-processor interaction, result-file contents, safe machine behavior, or successful execution on a target machine.
